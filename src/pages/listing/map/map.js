...
async function fetchRegionGeoJSON(longitude, latitude) {
  try {
    ...
    regionResult.innerHTML = `
      <h4>Region Info:</h4>
      <ul>
        <li><strong>District:</strong> ${data.features[0]?.properties?.district || "Unknown"}</li>
        <li><strong>Province:</strong> ${data.features[0]?.properties?.province || "Unknown"}</li>
        <li><strong>Sub-district:</strong> ${data.features[0]?.properties?.sub_district || "Unknown"}</li>
        <li><strong>Village:</strong> ${data.features[0]?.properties?.village || "Unknown"}</li>
      </ul>
    `;
